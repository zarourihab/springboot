...
public class OrganismeController {
    ...
    @PostMapping
    public OrganismeDTO create(@Valid @RequestBody OrganismeDTO dto) {
        return service.create(dto);
    }

    @GetMapping("/{id}")
    public OrganismeDTO getById(@PathVariable Long id) {
        return service.findById(id);
    }

    @GetMapping
    public List<OrganismeDTO> getAll(@RequestParam(required = false) String q) {
        return service.findAll(q);
    }

    @PutMapping("/{id}")
    public OrganismeDTO update(@PathVariable Long id, @Valid @RequestBody OrganismeDTO dto) {
